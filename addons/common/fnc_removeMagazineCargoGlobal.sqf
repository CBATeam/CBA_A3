...
Description:
    ...
    * Use <CBA_fnc_removeMagazineCargo> if you want to remove the item in local
      mission space only.
...
Examples:
   (begin example)
   // remove 1 Smokegrenade globally from the box
   _success = [myCoolWeaponBox, "SmokeShell"] call CBA_fnc_removeMagazineCargoGlobal;

   // remove 2 Handgrenades globally from the box
   _success = [myCoolWeaponBox, "HandGrenade_West", 2] call CBA_fnc_removeMagazineCargoGlobal;
   (end)
