...
Description:
    Removes specific magazine(s) from global cargo space. MP synchronized.
    Warning: Magazine's ammo count is lost and becomes full.

    * Use <CBA_fnc_removeMagazineCargo> if you want to remove the magazine(s) in local
      cargo space.
...
Examples:
   (begin example)
   // Remove 1 Smokegrenade globally from a box
   _success = [myCoolMagazineBox, "SmokeShell"] call CBA_fnc_removeMagazineCargoGlobal;

   // Remove 2 Handgrenades globally from a box
   _success = [myCoolMagazineBox, "HandGrenade_West", 2] call CBA_fnc_removeMagazineCargoGlobal;
   (end)
...
clearMagazineCargoGlobal _box;
{
    ...
    if (_x == _item) then {
        ...
        if (_itemCount > 0) then {
            // Add with new count
            _box addMagazineCargoGlobal [_x, _itemCount];
        };
    } else {
        // Readd only
        _box addMagazineCargoGlobal [_x, _itemCount];
