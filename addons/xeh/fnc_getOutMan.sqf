...
Examples:
    (begin example)
        ["CAManBase", {systemChat str _this}, true, []] call CBA_XEH_fnc_getOutMan;
    (end)
...
if (isNil QGVAR(getOutAllEvents)) then {
    GVAR(getOutAllEvents) = [];
    ...
    ["All", "getOut", {
        ...
        } forEach GVAR(getOutAllEvents);
    }] call CBA_fnc_addClassEventHandler;
};

GVAR(getOutAllEvents) pushBack [_className, _eventFunc, _allowInheritance, _excludedClasses];
