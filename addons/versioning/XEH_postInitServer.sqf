...
_str = 'if (isServer) exitWith{};\
    ...
    0 = objNull spawn {\
        sleep 2;\
        _func = {\
            GVAR(mismatch) = [format ["%2 (%1)", name player, player], _this];\
            publicVariable QGVAR(mismatch);\
            _this spawn {\
                _t = format ["You are missing the following mod: %1",_this];\
                diag_log text _t;\
                sleep 2;\
                if (CBA_display_ingame_warnings) then {\
                    player globalChat _t;\
                };\
            };\
        };';
...
[GVAR(versions_serv), {
    ...
    _addon = if (isClass _cfg) then {
        if (isText (_cfg >> "main_addon")) then {
            getText (_cfg >> "main_addon");
        } else {
            _key + "_main";
        };
    } else {
