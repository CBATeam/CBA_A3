...
{
	#ifdef DEBUG
		[format["readKeyFromConfig: %1, Found: %2", _this, getNumber(CFGSETTINGS >> _component >> _action)], STR(ADDON)] call CBA_fDebug;
	#endif
...
{
	#ifdef DEBUG
		[format["readKeyFromConfig: %1, Found: %2", _this, getNumber(CFGSETTINGS >> _component >> _action >> "key")], STR(ADDON)] call CBA_fDebug;
	#endif
	_i = 0;
	{
