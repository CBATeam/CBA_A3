...
Examples:
 ["your_mod", "your_action", "your_mod_fnc_openMenu", [15, true, true, true]]
      call cba_fnc_registerKeybindHandler;

Author:
 Taosenai
---------------------------------------------------------------------------- */

private ["_modName", "_actionName", "_functionName", "_defaultKeybind", "_overwrite", "_modKeybinds", "_keybind", "_dikCode", "_shift", "_ctrl", "_alt"];
...
_overwrite = [_this, 4, false, [false]] call bis_fnc_param;
...
// See if this action has already been mapped to a key handler.
_index = [_modName, _actionName] call FUNC(getKeyHandler);
if (_index > -1) then {
	// It's already mapped to a key handler, so it must be updated.

	// Grab the handler data from the array.
	_handlerData = (uiNamespace getVariable QGVAR(handlers)) select _index;
	...
	// Add new key handler with updated data.
	if (_dikCode != -1) then { // A DIK of -1 signifies "no key set"
		// Get a fresh event handler ID.
		_ehID = uiNamespace getVariable QGVAR(ehCounter);
		// Increment the event handler ID source.
		uiNamespace setVariable [QGVAR(ehCounter), _ehID + 1];
	...
	uiNamespace setVariable [QGVAR(handlers), _handlerTracker];
