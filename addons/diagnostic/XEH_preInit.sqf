...
FUNC(initExtendedDebug) = {
	...
	_y = -15 * ((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) + (safezoneY + safezoneH - (((safezoneW / safezoneH) min 1.2) / 1.2));
    ...
    for "_i" from 0 to (count _dbgConfig)-1 do {
        ...
        if(isClass(_control)) then {
            ...
            if(_idc != 0 && !(_idc in _exclude)) then {
                ...
                _pos set[1, (_pos select 1)+YPOS(15)];
	...
	_y = 12.8 * ((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) + (safezoneY + safezoneH - (((safezoneW / safezoneH) min 1.2) / 1.2));
