...
#define XEH_FUNC(A) SLX_XEH_EH_##A = { if ('A' in ['Respawn', 'MPRespawn', 'Killed', 'MPKilled']) then { diag_log ['A',_this, local (_this select 0)] }; {_this call _x}forEach((_this select 0)getVariable'Extended_##A##EH') }

SLX_XEH_EH_Init = { [_this select 0,'Extended_Init_EventHandlers']call SLX_XEH_init };
SLX_XEH_EH_RespawnInit = { diag_log ["YUUHUU",_this]; [_this select 0, "Extended_Init_EventHandlers", true] call SLX_XEH_init };
